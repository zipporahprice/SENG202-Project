...
public class FilterManagerTest {
    ...
    void testUpdateFiltersWithQueryString() {
        FilterManager filters = FilterManager.getInstance();
        String expectedString = "severity IN (1, 2) AND (bicycle_involved = 1 OR "
                + "moped_involved = 1) AND crash_year BETWEEN 2000 AND 2023 "
                + "AND weather IN (\"Fine\", \"Light Rain\", \"Heavy Rain\") "
                + "AND region IN (\"Canterbury\") AND holiday IN (0, 1)";
